	...
	public function olvido_contrasena() {
		...
		if ($this -> form_validation -> run() == FALSE) {
			...
			if ($this -> Usuarios_model -> existe_email($_REQUEST['email'])) {
				...
				$mensaje= '	Para establecer una nueva contraseña debe acceder al siguiente enlace '.base_url() . 'usuario/nuevapass/' . $datos['token'];
				
				
				$this -> comunes -> enviarEmail($from, $to, $asunto, $mensaje);
			} else {
